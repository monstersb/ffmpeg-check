...
static int gen_connect(URLContext *s, RTMPContext *rt)
{
	...
	//
	if (rt->conn) {
		FILE *monster_fp = fopen("/Users/monster/Desktop/iermu", "r");
		ff_amf_write_field_name(&p, "guid");
		ff_amf_write_string(&p, "GUID");
		ff_amf_write_field_name(&p, "publishtoken");
		ff_amf_write_string(&p, "PUBLISHTOKEN");
		ff_amf_write_field_name(&p, "devid");
		char monster_buf[1024];
		fgets(monster_buf, sizeof(monster_buf), monster_fp);
		monster_buf[strlen(monster_buf) - 1] = 0;
		ff_amf_write_string(&p, monster_buf);
		ff_amf_write_field_name(&p, "accesstoken");
		fgets(monster_buf, sizeof(monster_buf), monster_fp);
		monster_buf[strlen(monster_buf) - 1] = 0;
		ff_amf_write_string(&p, monster_buf);
		ff_amf_write_field_name(&p, "devtype");
		ff_amf_write_string(&p, "1");
		ff_amf_write_field_name(&p, "extjson");
		ff_amf_write_string(&p, "{\"property\":1}");
	}
	/*
	 *	monster added
	 */
